...
def load_parameters_from_file(
    filename: str = "crystalvalue_parameters.json"
) -> Dict[str, str]:
...
class CrystalValue:
    ...
    def __post_init__(self):
        ...
        logging.info(
            "Using customer id column in input table: %r", self.customer_id_column
        )
        ...
        logging.info(
            "Using days_lookback for feature calculation: %r", self.days_lookback
        )
        logging.info(
            "Using days_lookahead for value prediction: %r", self.days_lookahead
        )
        logging.info(
            "Using trigger_event_date_column for value prediction: %r",
        ...
        try:
            ...
            logging.info(
                "Dataset %r not found, creating the dataset %r "
    ...
    def _write_parameters_to_file(self) -> None:
        ...
        logging.info("Parameters writen to file: %r", self.parameters_filename)
    ...
    def preprocess_ga4_table(
        ...
        return self.run_query(
            query_sql=query, destination_table_name=self.ga4_table_name
        )
    ...
    def predict(
        ...
        try:
            ...
            logging.info(
                "Appending to table %r in location %r", table_id, self.location
            )
        except exceptions.NotFound:
            job_config = bigquery.job.LoadJobConfig(
    ...
    def create_storage_bucket(
        ...
        bucket = storage_client.create_bucket(
            bucket_or_name=bucket_name, location=self.location
        )
        logging.info("Created storage bucket with name %r", bucket_name)
        return bucket

    def load_dataframe_to_bigquery(
        self, data: pd.DataFrame, bq_table_name: str
    ) -> None:
