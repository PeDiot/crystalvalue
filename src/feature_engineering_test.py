...
class FeatureEngineeringTest(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_read = mock.patch.object(
            feature_engineering, "_read_file", autospec=True
        ).start()
        self.mock_write = mock.patch.object(
            feature_engineering, "_write_file", autospec=True
        ).start()
    ...
    def test_build_query_substitutions(self):
        with mock.patch.object(
            ...
            expected_boolean_str = (
                "MAX(CAST(boolean_column AS INT))" " AS max_boolean_column"
            )
            expected_string_str = (
                "COUNT (DISTINCT string_column)" " AS n_distinct_string_column"
            )
