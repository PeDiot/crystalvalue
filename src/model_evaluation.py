...
def _calculate_normalized_mae(y_actual: pd.Series, y_predicted: pd.Series) -> float:
...
def _gini(series1: pd.Series, series2: pd.Series) -> np.float64:
    ...
    """
    y_all = np.asarray(np.c_[series1, series2, np.arange(len(series1))], dtype=float)
...
def _compute_gini_normalized(y_actual: pd.Series, y_predicted: pd.Series) -> float:
