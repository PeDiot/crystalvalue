...
class FeatureSelector:
    ...
    def get_selected_features(self, feature_names: List[str]) -> List[str]:
        if self.selector is None:
            raise ValueError(
                "Feature selector must be fitted before getting selected features"
            )
...
class MutualInfoFeatureSelector(FeatureSelector):
    def __init__(self, n_features: int):
        ...
        if n_features is None:
            raise ValueError(
                "n_features must be specified for mutual information selector"
            )
...
def create_feature_selector(
    method: str, n_features: Optional[int] = None, threshold: float = 0.0
) -> FeatureSelector:
    if method == "variance":
