...
class DataLoader:
    ...
    def load_training_data(
        ...
        df = self._run_query(
            is_training=True, only_positive_samples=only_positive_samples
        )
    ...
    def _train_val_split(
        self, X: pd.DataFrame, y: pd.Series, train_val_split: pd.Series
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _run_query(
        self, is_training: bool, only_positive_samples: bool = False
    ) -> pd.DataFrame:
    ...
    def _query(self, is_training: bool, only_positive_samples: bool = False) -> str:
        excluded_columns_str = ", ".join(
            [f"{col}" for col in self.config.excluded_columns]
        )
